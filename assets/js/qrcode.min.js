 ...
 * Fixed Drawing class to properly create canvas elements
 */
var QRCode;
(function () {
	...
	for (var i = 8; i < 256; i++) QRMath.EXP_TABLE[i] = QRMath.EXP_TABLE[i - 4] ^ QRMath.EXP_TABLE[i - 5] ^ QRMath.EXP_TABLE[i - 8] ^ QRMath.EXP_TABLE[i - 1];
	...
	// Auto type number calculation
	var QR_CAPACITY_TABLE = [
		...
		[425, 331, 241, 177], [458, 362, 258, 194], [520, 412, 292, 220], [586, 450, 322, 247],
		[644, 504, 360, 280], [718, 560, 400, 308], [792, 624, 444, 338], [858, 666, 482, 364],
		[929, 711, 509, 388], [1003, 779, 554, 418], [1091, 857, 607, 453], [1171, 911, 647, 481],
		[1273, 997, 709, 521], [1367, 1059, 751, 551], [1465, 1125, 805, 585], [1528, 1190, 848, 611],
		[1664, 1264, 908, 661], [1766, 1334, 958, 697], [1892, 1426, 1016, 742], [2012, 1502, 1074, 790],
		[2138, 1582, 1136, 838], [2238, 1666, 1202, 886], [2369, 1748, 1258, 934], [2476, 1834, 1322, 982],
		[2600, 1924, 1390, 1030], [2728, 2018, 1458, 1088], [2848, 2116, 1530, 1146], [2976, 2218, 1602, 1206]
	];
	...
	function _getTypeNumber(sText, nCorrectLevel) {
		...
		for (var i = 0, len = QR_CAPACITY_TABLE.length; i <= len; i++) {
			var nLimit = 0;
			switch (nCorrectLevel) {
				case QRErrorCorrectLevel.L: nLimit = QR_CAPACITY_TABLE[i][0]; break;
				case QRErrorCorrectLevel.M: nLimit = QR_CAPACITY_TABLE[i][1]; break;
				case QRErrorCorrectLevel.Q: nLimit = QR_CAPACITY_TABLE[i][2]; break;
				case QRErrorCorrectLevel.H: nLimit = QR_CAPACITY_TABLE[i][3]; break;
			}
			if (nLimit >= length) break;
			nType++;
		}
		if (nType > QR_CAPACITY_TABLE.length) throw new Error("Too long data");
		return nType;
	}

	// ============================================================
	// Drawing - creates a CANVAS element inside the container div
	// ============================================================
	...
	Drawing.prototype.draw = function (oQRCode) {
		...
		var tileW = _htOption.width / nCount;
		var tileH = _htOption.height / nCount;
		...
		try {
			...
			img.alt = "QR Code";
			img.width = _htOption.width;
			img.height = _htOption.height;
	...
	Drawing.prototype._drawTable = function (oQRCode) {
		var _htOption = this._htOption;
		var _el = this._el;
		var nCount = oQRCode.getModuleCount();
		var nWidth = Math.floor(_htOption.width / nCount);
		var nHeight = Math.floor(_htOption.height / nCount);
		var aHTML = ['<table style="border:0;border-collapse:collapse;padding:0;margin:0 auto;">'];
		for (var row = 0; row < nCount; row++) {
			aHTML.push('<tr>');
			for (var col = 0; col < nCount; col++) {
