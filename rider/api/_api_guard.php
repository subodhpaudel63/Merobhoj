<?php
/**
 * Shared guard + helpers for every rider API endpoint.
 * Include FIRST. Enforces JSON output, rider role (admin override), and gives
...
// 401 JSON (require_role detects the /api/ path) if not rider or admin.
$panelUser = require_role($conn, 'rider');

/**
 * The acting rider's user id — taken from the DB-revalidated session record,
 * NEVER from the request body. Every ownership check binds this value.
 */
$RIDER_ID = (int)($panelUser['id'] ?? 0);
