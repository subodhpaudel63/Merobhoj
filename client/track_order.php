...
if (!$currentUser) {
    $_SESSION['msg'] = ['type' => 'error', 'text' => 'Please login to track your order.'];
...
// Track target may arrive via ?order=<orderNumber> (sent from the orders list).
// When omitted, client-side myorderPickTracked() falls back to the user's most
// recent active order, mirroring the old standalone client/track_order.php.
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    ...
    <header>
      <div class="container header my-3 d-none d-lg-flex">
        ...
        <!-- ============ END HEADER ============ -->
    <!--
      Inline SVG icon definitions referenced by <use href="#ic-…"> in the
      order stepper, the live map and the help/footer bars. Sourced from
      stock Material icons so the standalone tracking page is self-contained.
    -->
    ...
    <!-- =================================================================
         VIEW DETAILS MODAL — filled by openOrderDetails(id). Present here so
         the "View Bill / Order Details" button on the tracking page works.
         ================================================================= -->
