<?php
/**
 * Shared panel top header — Chef.
 * Mirrors admin/topbar.php markup/IDs so assets/js/adminscript.js wires up
 * the sidebar toggle, theme toggle and profile dropdown with no extra JS.
 * Expects $panelUser (set by require_role() on the including page); falls
 * back to current_panel_user() defensively.
 */
if (!isset($panelUser) || !is_array($panelUser)) {
    $panelUser = current_panel_user($conn) ?? ['name' => 'Chef', 'email' => '', 'user_type' => 'chef', 'user_img' => ''];
}

$panelName  = trim((string)($panelUser['name'] ?? '')) ?: 'Kitchen Chef';
$panelRole  = (string)($panelUser['user_type'] ?? 'chef');
...
$initials = strtoupper(substr($parts[0] ?? 'C', 0, 1) . (isset($parts[1]) ? substr($parts[1], 0, 1) : ''));
?>
<script>
...
<header class="admin-topbar" id="admin_topbar">
    ...
    <div class="admin-topbar-actions">
        ...
        <!-- Voice Announcements Toggle (KDS) — off by default, persisted in localStorage -->
        <button type="button" class="panel-voice-toggle" id="kdsVoiceToggle" aria-label="Toggle voice announcements" title="Voice announcements for new orders">
            <span class="material-symbols-sharp">volume_off</span>
        </button>
